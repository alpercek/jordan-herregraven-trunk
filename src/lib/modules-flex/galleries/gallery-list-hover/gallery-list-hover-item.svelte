...
<div class="hidden sm:block pb-20" style="margin-left: {marginLeft}%; width: {itemWidth}%; padding-bottom: {marginBottom}vw">
    {#if item.gallery_image_link[0]}
        ...
        <a data-sveltekit-prefetch href="{item.gallery_image_link[0].text}">
...
{#if item.gallery_image_link[0]}
    <!-- With link -->
    <a data-sveltekit-prefetch href="{item.gallery_image_link[0].text}" class="relative block sm:hidden text-white uppercase text-xs tracking-widest leading-4">
        {#if item.gallery_image.Big.url}
